...
ig.module('plugins.box2d.lib').defines(function(){
    ...
    // Trouble section for PiSpace
    // Very fast objects?
    a.DestroyProxy=function(b){
	for(var c,d,e=this.m_proxyPool[b],f=2*this.m_proxyCount,g=0;g<2;++g){
	    // Issue with j.  Can be 0 which causes failure in later test.
		...
		o=m.length; 
		q=j-1;
		// Added for reoccuring error found in PiSpace
		// j would be -1 which would cause c to be undefined.
		// Simple test fix is to set j to 0 if it is -1 or to skip.
		// FAIL!
		for(n=0;n<o;n++){
		    d=m[n]; 
		    c=h[parseInt(q+n)]; 
