...
.defines(function(){
	...
	menuText: [
	    ...
	    'j/k to add/kill pucks',
	    'n to change the shape of the puck'
	],
	building: true,
	hud: 16,
	...
	init: function() {
	    ...
	    // DEBUG Spawn/Destroy Puck
	    ig.input.bind( ig.KEY.J, 'addpuck' );
	    ig.input.bind( ig.KEY.K, 'killpuck' );
	    ig.input.bind( ig.KEY.N, 'shapeShift' );
	    //ig.input.bind( ig.KEY.B, 'superSize' );
	...
	update: function() {
		...
		if (this.mode == PiSpace.MODE.TITLE) {
		    ...
		    if (ig.input.pressed('addpuck')){
			this.spawnEntity( EntityPuck, ig.system.width / 2, ig.system.height / 2 + 16 );
		    } else if (ig.input.pressed('killpuck')){
			pucks = this.getEntitiesByType( EntityPuck );
			if (pucks[1] != undefined){
			    toKill = pucks.shift();
			    toKill.kill();
			}
		    }
	...
	buildSPGame: function(){
	    ...
	    ig.game.spawnEntity( EntityPaddlePlayer, ig.system.width - 24, ig.system.height / 2 - 20 );
	    ig.game.spawnEntity( EntityPaddleCpu, 16, ig.system.height / 2 - 20 );
	...
	buildMPGame: function(){
	    ...
	    ig.game.spawnEntity( RightWall, ig.system.width - 8 , this.hud );
	    ig.game.spawnEntity( EntityPaddlePlayer, ig.system.width - 24, ig.system.height / 2 - 20);
	    ig.game.spawnEntity( EntityPaddlePlayer2, 16, ig.system.height / 2 - 20 );
	    ig.game.spawnEntity( EntityPuck, ig.system.width / 2, ig.system.height / 2 + 16 );
	    return;
	},
	...
	buildAIGame: function(){
	    ...
	    ig.game.spawnEntity( LeftWall, 0, this.hud );
	    ig.game.spawnEntity( RightWall, ig.system.width - 8 , this.hud );
	    ig.game.spawnEntity( EntityPaddleCpu2, ig.system.width - 24, ig.system.height / 2 - 20 );
	    ig.game.spawnEntity( EntityPaddleCpu, 16, ig.system.height / 2 - 20 );
	    ig.game.spawnEntity( EntityPuck, ig.system.width / 2, ig.system.height / 2 + 16 );
	    return;
	},
	
	// Reset canvas to prepare for new game mode
	...
	draw: function(){
		...
		*/
		...
		if (this.mode == PiSpace.MODE.SP){
		    ...
		    this.font.draw('HEAT', (ig.system.width - 43) * ig.system.scale, 
				   10 * ig.system.scale, ig.Font.ALIGN.RIGHT
				  );
	...
	drawHeatBar: function ( info ){
	    ...
	    var test = parseInt(100 - info.val);
	    
	    if (test > 100){
		test = 100;
	    }
	    ig.system.context.fillStyle = this.picker.hexToRGBstr( this.picker.triColors[parseInt(test)] );
	    	    
	    // draw heat bar
	    ig.system.context.fillRect( info.loc.x, info.loc.y,
					(40 * ig.system.scale) * per,
					5 * ig.system.scale
				      );
	}
