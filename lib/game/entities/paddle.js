...
.defines(function(){
	...
	vectorLeft: function(){
	    this.body.ApplyImpulse( new b2.Vec2(-0.3*this.burn,0), this.body.GetPosition() );
	    //ig.game.spawnEntity( FireGib, this.pos.x + 8, this.pos.y + Math.random()*40 );
	    return;
	},
	
	vectorRight: function(){
	    this.body.ApplyImpulse( new b2.Vec2(0.3*this.burn,0), this.body.GetPosition() );
	    //ig.game.spawnEntity( FireGib, this.pos.x, this.pos.y + Math.random()*40 );
	    return;
	},
	
	vectorUp: function(){
	    this.body.ApplyImpulse( new b2.Vec2(0,-0.3*this.burn), this.body.GetPosition() );
	    //ig.game.spawnEntity( FireGib, this.pos.x + Math.random()*8, this.pos.y + 40);
	    return;
	},
	
	vectorDown: function(){
	    this.body.ApplyImpulse( new b2.Vec2(0,0.3*this.burn), this.body.GetPosition() );
	    //ig.game.spawnEntity( FireGib, this.pos.x + Math.random()*8, this.pos.y );
	    return;
	},
	...
	checkCollisions: function() {
		...
		if( edge.other.entity && edge.other.userData != 'particle') {
			...
			if (edge.other.entity.name == 'puck'){
