...
.defines(function(){
	...
	verts: [ [0,0], [8,0], [8,40], [0,40] ],
	b2verts: [ new b2.Vec2(0,0), new b2.Vec2(8,0), new b2.Vec2(8,40), new b2.Vec2(0,40) ],
	...
	vectorRight: function(){
	    ...
	    if (this.burn > 1){
		ig.game.spawnEntity( BlueFireGib, this.pos.x, this.pos.y + Math.random()*40 );
		ig.game.spawnEntity( BlueFireGib, this.pos.x, this.pos.y + Math.random()*40 );
		this.fuel -= this.burn;
	    }
	    return;
	...
	vectorUp: function(){
	    ...
	    if (this.burn > 1){
		ig.game.spawnEntity( BlueFireGib, this.pos.x + Math.random()*8, this.pos.y + 40);
		ig.game.spawnEntity( BlueFireGib, this.pos.x + Math.random()*8, this.pos.y + 40);
		this.fuel -= this.burn;
	    }
	    return;
	...
	vectorDown: function(){
	    this.body.ApplyImpulse( new b2.Vec2(0,0.2*this.burn), this.body.GetPosition() );
	    if (this.burn > 1){
		ig.game.spawnEntity( BlueFireGib, this.pos.x + Math.random()*8, this.pos.y );
		ig.game.spawnEntity( BlueFireGib, this.pos.x + Math.random()*8, this.pos.y );
		this.fuel -= this.burn;
